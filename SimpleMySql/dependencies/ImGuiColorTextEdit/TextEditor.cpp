...
std::string TextEditor::GetCurrentLineText(int offset)const
{
	auto lineLength = GetLineMaxColumn(mState.mCursorPosition.mLine + offset);
	return GetText(
		Coordinates(mState.mCursorPosition.mLine + offset, 0),
		Coordinates(mState.mCursorPosition.mLine + offset, lineLength));
}
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::MySQL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
